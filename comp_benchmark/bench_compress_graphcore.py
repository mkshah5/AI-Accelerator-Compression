...
from compressor.compress_entry_f32 import compress_on_device, decompress, get_lhs_rhs_compress
...
BENCHMARK_NAME = "compressstandalone"
VERSION = "graphcore"
...
class CompressorModel(nn.Module):
    ...
    # assume bs > 1
    def forward(self, x):
        r = compress_on_device(torch.squeeze(x[:,0,:,:]), self.lhs.ipu(),self.rhs.ipu())
        g = compress_on_device(torch.squeeze(x[:,1,:,:]), self.lhs.ipu(),self.rhs.ipu())
        b = compress_on_device(torch.squeeze(x[:,2,:,:]), self.lhs.ipu(),self.rhs.ipu())
...
def prepare_fulldata(args: argparse.Namespace, opts_t) -> Tuple[torch.utils.data.DataLoader]:
    ...
    features = torch.randn([TRAIN_SIZE*10, PARAMS.nchannels, RPIX, CPIX])
