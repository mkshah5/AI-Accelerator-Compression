...
def train(args: argparse.Namespace, model: nn.Module, optimizer: poptorch.optim.SGD) -> None:
    ...
    train_loader = get_data_generator(Path(DATA_DIR), TRAIN_SIZE, is_inference=False, opts_t=opts_t,opts_v=opts_v)
    test_loader = get_data_generator(Path(DATA_DIR), TRAIN_SIZE, is_inference=True, opts_t=opts_t,opts_v=opts_v)
